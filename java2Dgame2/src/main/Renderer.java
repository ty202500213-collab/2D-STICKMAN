...
public class Renderer {
    ...
    public Renderer(int screenWidth, int screenHeight) {
        ...
        String[] paths = {
            "resources/boxing_bg.jpg",
            "../../resources/boxing_bg.jpg",
            "java2Dgame2/resources/boxing_bg.jpg"
        };
    ...
    public void drawGame(Graphics2D g2, Player p1, Player p2, GameState state,
        ...
        for (int[] p : state.particles) {
            ...
            int tag = (p.length > 5) ? p[5] : 0;
            if (tag == 1) {
                // Golden sparks for uppercut
                g2.setColor(new Color(1f, 0.92f, 0.1f, alpha));
                int sz = 6 + (int)(alpha * 6);
                g2.fillOval(p[0] - sz/2, p[1] - sz/2, sz, sz);
                g2.setColor(new Color(1f, 0.6f, 0f, alpha * 0.5f));
                g2.drawOval(p[0] - sz/2 - 2, p[1] - sz/2 - 2, sz + 4, sz + 4);
            } else if (tag == 2) {
                // Cyan swirl for tornado kick
                g2.setColor(new Color(0.1f, 0.9f, 1f, alpha));
                int sz = 5 + (int)(alpha * 7);
                g2.fillOval(p[0] - sz/2, p[1] - sz/2, sz, sz);
                g2.setColor(new Color(1f, 1f, 1f, alpha * 0.5f));
                g2.drawOval(p[0] - sz/2 - 2, p[1] - sz/2 - 2, sz + 4, sz + 4);
            } else {
        ...
        drawStickman(g2, (int)p1.x, (int)p1.y, p1.facing, p1.anim, p1.flashTimer > 0, 1, p1.isMoving, p1.isBlocking, p1.blockFlashTimer > 0);
        drawStickman(g2, (int)p2.x, (int)p2.y, p2.facing, p2.anim, p2.flashTimer > 0, 2, p2.isMoving, p2.isBlocking, p2.blockFlashTimer > 0);
    ...
    private void drawStickman(Graphics2D g2, int x, int y, int facing, String anim,
                               ...
                               boolean isBlocking, boolean blockFlash) {
        Color bodyColor    = blockFlash ? new Color(100, 200, 255)
                           : flash     ? new Color(255, 70, 70)
                           : (playerNum == 1 ? new Color(100, 180, 255) : new Color(255, 130, 100));
        Color outlineColor = blockFlash ? new Color(50, 120, 255)
                           : flash     ? new Color(255, 30, 30)
        ...
        if (anim.equals("block")) {
            // Block stance: arms crossed in front forming a guard, slight forward lean
            int kd = facing;
            // Body slightly forward-leaning
            g2.setColor(bodyColor); g2.setStroke(thick);
            g2.drawLine(x, y - 35, x + kd * 6, y - 68); // torso slight lean toward opponent
            // Both arms raised and crossed in a guard
            int guardX = x + kd * 22;
            int guardY = y - 70;
            // Upper arm (across face)
            g2.drawLine(x + kd * 6, y - 68, guardX + kd * 8, guardY - 10);
            g2.drawLine(x + kd * 6, y - 68, guardX,          guardY + 8);
            // Draw crossed guard gloves
            g2.setColor(gloveColor);
            g2.fillOval(guardX + kd * 8 - 9, guardY - 10 - 9, 18, 18);
            g2.fillOval(guardX - 9,           guardY + 8  - 9, 18, 18);
            g2.setColor(new Color(50, 150, 255)); // blue shield tint on gloves
            g2.setStroke(new BasicStroke(2f));
            g2.drawOval(guardX + kd * 8 - 11, guardY - 10 - 11, 22, 22);
            g2.drawOval(guardX - 11,           guardY + 8  - 11, 22, 22);
            // Draw a faint shield aura
            if (isBlocking) {
                g2.setColor(new Color(80, 180, 255, 55));
                g2.fillOval(x - 30, y - 110, 60, 100);
                g2.setColor(new Color(120, 210, 255, 90));
                g2.setStroke(new BasicStroke(2.5f));
                g2.drawOval(x - 30, y - 110, 60, 100);
            }
        } else if (anim.equals("stagger")) {
            // Stagger: body bent back, arms flailing out to sides
            int kd = facing;
            // Body tilted back — draw spine angled
            int bodyTopX = x - kd * 10, bodyTopY = y - 60;
            g2.setColor(bodyColor); g2.setStroke(thick);
            g2.drawLine(x, y - 35, bodyTopX, bodyTopY);  // torso tilts back
            // Arms flung out
            g2.drawLine(bodyTopX, bodyTopY, bodyTopX - 28, bodyTopY - 10);  // back arm up
            g2.drawLine(bodyTopX, bodyTopY, bodyTopX + 22, bodyTopY + 14);  // front arm down
            // Gloves
            g2.setColor(gloveColor);
            g2.fillOval(bodyTopX - 28 - 7, bodyTopY - 10 - 7, 14, 14);
            g2.fillOval(bodyTopX + 22 - 7, bodyTopY + 14 - 7, 14, 14);
        } else if (anim.equals("tornadoKick")) {
            // Tornado kick: airborne spin — body horizontal, one leg extended wide forward
            int kd = facing;
            // Body nearly horizontal (spinning in air)
            int bodyTopX = x + kd * 8, bodyTopY = y - 55;
            g2.setColor(bodyColor); g2.setStroke(thick);
            g2.drawLine(x, y - 35, bodyTopX, bodyTopY);  // torso forward lean
            // Arms back for spin momentum
            g2.drawLine(bodyTopX, bodyTopY, bodyTopX - kd * 24, bodyTopY - 8);
            g2.drawLine(bodyTopX, bodyTopY, bodyTopX - kd * 18, bodyTopY + 14);
            g2.setColor(gloveColor);
            g2.fillOval(bodyTopX - kd * 24 - 7, bodyTopY - 8 - 7, 14, 14);
            g2.fillOval(bodyTopX - kd * 18 - 7, bodyTopY + 14 - 7, 14, 14);
            // Kick leg extended far forward — the spinning kick
            int thighX = x + kd * 18, thighY = y - 18;
            int footX  = x + kd * 72, footY  = y - 42;  // wide horizontal kick
            g2.setColor(bodyColor);
            g2.drawLine(x, y - 35, thighX, thighY);
            g2.drawLine(thighX, thighY, footX, footY);
            // Kick glow (cyan)
            g2.setColor(new Color(0, 220, 255));
            g2.setStroke(new BasicStroke(3f));
            g2.drawOval(footX - 12, footY - 12, 24, 24);
            g2.setColor(new Color(180, 255, 255, 160));
            g2.fillOval(footX - 10, footY - 10, 20, 20);
            // Standing leg tucked
            g2.setColor(bodyColor); g2.setStroke(thick);
            g2.drawLine(x, y - 35, x - kd * 10, y - 12);
            g2.drawLine(x - kd * 10, y - 12, x - kd * 8, y);
        } else if (anim.equals("uppercut")) {
            // Uppercut: one arm swings upward in a rising arc
            int pd = facing;
            // Back arm tucked
            int bx2 = x - pd * 18, by2 = y - 52;
            g2.setColor(bodyColor); g2.setStroke(thick);
            g2.drawLine(x, y - 65, bx2, by2);
            g2.drawLine(bx2, by2, bx2 - pd * 5, by2 - 5);
            g2.setColor(gloveColor); g2.fillOval(bx2 - 7 - pd * 5, by2 - 12, 14, 14);
            g2.setColor(gloveHL);   g2.drawOval(bx2 - 7 - pd * 5, by2 - 12, 14, 14);
            // Uppercut arm: elbow pulled back then fist thrusts UP and forward
            g2.setColor(bodyColor); g2.setStroke(thick);
            int elbX = x + pd * 14, elbY = y - 55;
            int fistX = x + pd * 28, fistY = y - 105; // fist goes HIGH up
            g2.drawLine(x, y - 65, elbX, elbY);
            g2.drawLine(elbX, elbY, fistX, fistY);
            // Draw glowing uppercut fist
            Color ucGlow = new Color(255, 220, 60);
            g2.setColor(gloveColor); g2.fillOval(fistX - 10, fistY - 10, 20, 20);
            g2.setColor(ucGlow);     g2.setStroke(new BasicStroke(2.5f));
            g2.drawOval(fistX - 13, fistY - 13, 26, 26);  // glow ring
        } else if (anim.equals("punch")) {
            int pd = facing;
