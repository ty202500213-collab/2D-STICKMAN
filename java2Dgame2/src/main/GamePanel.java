...
public class GamePanel extends JPanel implements Runnable {
    ...
    void update() {
        ...
        boolean p1down  = keysPressed.contains(KeyEvent.VK_S);
        boolean p1punch = keysPressed.contains(KeyEvent.VK_H);
        boolean p1kick  = keysPressed.contains(KeyEvent.VK_J);
        boolean p1block = keysPressed.contains(KeyEvent.VK_G);

        boolean p2left  = keysPressed.contains(KeyEvent.VK_LEFT);
        boolean p2right = keysPressed.contains(KeyEvent.VK_RIGHT);
        boolean p2down  = keysPressed.contains(KeyEvent.VK_DOWN);
        boolean p2punch = keysPressed.contains(KeyEvent.VK_NUMPAD4);
        boolean p2kick  = keysPressed.contains(KeyEvent.VK_NUMPAD5);
        boolean p2block = keysPressed.contains(KeyEvent.VK_NUMPAD6);

        player1.update(p1left, p1right, p1down, p1punch, p1kick, p1block, player2);
        player2.update(p2left, p2right, p2down, p2punch, p2kick, p2block, player1);

        // Spawn uppercut effects when hit lands
        if (player2.uppercutHitThisFrame) {
            gameStateObj.spawnUppercutParticles((int)player2.x, (int)player2.y - 60);
            gameStateObj.addHitLabel((int)player2.x, (int)player2.y - 120, "UPPERCUT!");
        }
        if (player1.uppercutHitThisFrame) {
            gameStateObj.spawnUppercutParticles((int)player1.x, (int)player1.y - 60);
            gameStateObj.addHitLabel((int)player1.x, (int)player1.y - 120, "UPPERCUT!");
        }
        // Spawn tornado kick effects
        if (player2.tornadoKickHitThisFrame) {
            gameStateObj.spawnTornadoParticles((int)player2.x, (int)player2.y - 50);
            gameStateObj.addHitLabel((int)player2.x, (int)player2.y - 120, "TORNADO KICK!");
        }
        if (player1.tornadoKickHitThisFrame) {
            gameStateObj.spawnTornadoParticles((int)player1.x, (int)player1.y - 50);
            gameStateObj.addHitLabel((int)player1.x, (int)player1.y - 120, "TORNADO KICK!");
        }
