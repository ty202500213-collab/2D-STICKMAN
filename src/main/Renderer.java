...
public class Renderer {
    ...
    public void drawGame(Graphics2D g2, Player p1, Player p2, GameState state,
                         ...
                         Rectangle pauseBtn, boolean hoveringPause, boolean paused) {
        int w = screenWidth, h = screenHeight;
        ...
        drawPauseButton(g2, pauseBtn, hoveringPause, paused);
        ...
        if (paused) {
            ...
            String ps = "PAUSED";
            FontMetrics fm = g2.getFontMetrics();
            int px = (screenWidth - fm.stringWidth(ps)) / 2;
            g2.setColor(new Color(255, 220, 60));
            g2.drawString(ps, px, screenHeight / 2 - 10);
            g2.setFont(new Font("Courier New", Font.PLAIN, 14));
            g2.setColor(new Color(200, 200, 200));
            String hint = "Click \u23F5 or press P / ESC to resume";
            FontMetrics fmh = g2.getFontMetrics();
            g2.drawString(hint, (screenWidth - fmh.stringWidth(hint)) / 2, screenHeight / 2 + 22);
            // Redraw buttons on top of overlay so they remain visible
            drawPauseButton(g2, pauseBtn, hoveringPause, true);
            drawExitButton(g2, exitBtn, hoveringExit);
        }
    }
    ...
    public void drawGameOver(Graphics2D g2, Player p1, Player p2, GameState state,
                             ...
                             int timer, boolean infinite, int fps,
                             Rectangle exitBtn, boolean hoveringExit, Rectangle pauseBtn) {
        drawGame(g2, p1, p2, state, timer, infinite, fps, exitBtn, hoveringExit, pauseBtn, false, false);
    ...
    // --------------------------------------------------------- SHARED HELPERS
    private void drawPauseButton(Graphics2D g2, Rectangle btn, boolean hover, boolean paused) {
        Color bg     = paused  ? new Color(60, 120, 60)
                     : hover   ? new Color(70, 70, 160)
                               : new Color(25, 25, 70);
        Color border = paused  ? new Color(100, 220, 100)
                     : hover   ? new Color(150, 150, 255)
                               : new Color(70, 70, 140);
        g2.setColor(bg);
        ...
        g2.setFont(new Font("Courier New", Font.BOLD, 12));
        g2.setColor(hover || paused ? Color.WHITE : new Color(180, 180, 240));
        String label = paused ? "\u25B6 RESUME" : "\u23F8 PAUSE";
        FontMetrics fm = g2.getFontMetrics();
        g2.drawString(label,
