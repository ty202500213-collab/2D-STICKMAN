...
public class GamePanel extends JPanel implements Runnable {
    ...
    Rectangle pauseBtn    = new Rectangle(screenWidth / 2 - 44, 52, 88, 28);
    ...
    void resumeGame() {
        ...
        keysPressed.clear();
        // Wait briefly for any existing thread to exit its loop
        if (gameThread != null && gameThread.isAlive()) {
            try { gameThread.join(200); } catch (InterruptedException ignored) {}
        }
    ...
    void handleHover(Point p) {
        ...
        hoveringExit     = (gameState == STATE_GAME || gameState == STATE_GAMEOVER || gameState == STATE_PAUSED) && exitBtn.contains(p);
        hoveringPause    = (gameState == STATE_GAME || gameState == STATE_PAUSED) && pauseBtn.contains(p);

        boolean anyHover = hoveringPlay || hoveringPlayAgain || hoveringControls || hoveringQuit || hovering60 || hovering99
                        || hoveringInf || hoveringStart || hoveringBack || hoveringExit || hoveringPause;
    ...
    protected void paintComponent(Graphics g) {
        ...
        switch (gameState) {
            ...
            case STATE_GAME     -> renderer.drawGame(g2, player1, player2, gameStateObj,
                                        ...
                                        pauseBtn, hoveringPause, false);
            case STATE_PAUSED   -> renderer.drawGame(g2, player1, player2, gameStateObj,
                                        timerFrames, timerInfinite, FPS, exitBtn, hoveringExit,
                                        pauseBtn, hoveringPause, true);
